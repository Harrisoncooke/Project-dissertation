...
public class PerlinNoise
{
    ...
    public PerlinNoise()
    {
        // Initialize the permutation array with a fixed set of values
        permutation = new int[]
        {
        ...
        // Repeat the permutation array to avoid buffer overflow
        p = new int[permutation.Length * 2];
        for (int i = 0; i < permutation.Length; i++)
        {
            p[i] = permutation[i];
            p[i + permutation.Length] = permutation[i];
        }
    }
    ...
    // Gradient function to calculate dot product between a pseudorandom gradient vector and the distance vector
    ...
    // Generate Perlin noise value for a given point (x, y, z)
    public float Noise(float x, float y, float z)
    {
        ...
        // Calculate relative coordinates within the unit cube
